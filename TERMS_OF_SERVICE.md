# Terms of Service

_Last updated: 2026_

By registering a subdomain through this service, you agree to the following terms.

---

## 1. Permitted Use

Subdomains are provided free of charge for **personal or open-source projects** only.
Commercial use is allowed as long as it does not violate any other term listed here.

## 2. Prohibited Content

You may **not** use a subdomain to host, link to, or promote:

- Phishing, fraud, or social engineering
- Malware, ransomware, or any malicious software
- Spam or unsolicited communications
- Illegal content of any kind
- Hate speech, harassment, or content targeting individuals
- Content that impersonates another person or brand

## 3. Subdomain Per Person

Each GitHub account may register a maximum of **2 subdomains** unless an exception is
explicitly granted by a maintainer.

## 4. Ownership

You retain full ownership of the content hosted at your subdomain.
We do not claim any rights over your content.
We accept no liability for third-party websites and have no control over their content.

## 5. Termination

We reserve the right to remove any subdomain **without prior notice** if it violates
these terms, upon receiving a valid abuse report, or at our sole discretion.

## 6. No Warranty

This service is provided **"as is"** without any warranties, express or implied.
We make no guarantees regarding uptime, availability, or DNS propagation times.

## 7. Changes

These terms may be updated at any time. Continued use of the service after changes
constitutes acceptance of the new terms.
